...
void RZGetGaussianBlurWeights(GLfloat **weights, GLint *n, GLfloat sigma, GLint radius);
...
- (void)setSigma:(GLfloat)sigma
{
    ...
    for ( i = (GLint)self.blurs.count - 1, downsample = 0; downsample == 0 || remainingSigma > 0.0f; i--, downsample = MIN(downsample + 1, RZ_EFFECT_MAX_DOWNSAMPLE) ) {
        ...
        if ( i >= 0 ) {
            ((RZBlurEffect *)self.blurs[i]).sigma = levelSigma;
        }
        else {
            [self.blurs insertObject:[self rz_blurWithSigma:levelSigma downsample:downsample] atIndex:0];
        }
...
- (RZBlurEffectFull *)rz_blurWithSigma:(GLfloat)sigma downsample:(GLuint)downsample
{
...
void RZGetGaussianBlurWeights(GLfloat **weights, GLint *n, GLfloat sigma, GLint radius)
{
    ...
    *weights = (GLfloat *)malloc(numWeights * sizeof(GLfloat));
    ...
    if ( sigma >= kRZBlurEffectMinSigma ) {
